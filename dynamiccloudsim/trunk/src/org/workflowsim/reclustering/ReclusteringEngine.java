...
public class ReclusteringEngine {
    ...
    private static Job createJob(int id, Job job, long length, List taskList, boolean updateDep) {
        try {
            Job newJob = new Job(job.getUserId(),id, length);
    ...
    public static List<Job> process(Job job, int id) {        
        ...
        try {
            ...
            switch (FailureParameters.getFTCluteringAlgorithm()) {
                ...
                case FTCLUSTERING_SSR:
                	jobList = SSRReclustering(jobList, job, id);
                default:
                    break;
            }
        } catch (Exception e) {
    ...
    private static List<Job> SSRReclustering(List<Job> jobList, Job job, int id) {
        ...
        Job newJob = createJob(id, job, length, newTaskList, true);
