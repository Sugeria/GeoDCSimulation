...
//				}
				...
				for(int taskindex = 0; taskindex < numberOfTask; taskindex++) {
					...
					for(int dcindex = 0; dcindex < Parameters.numberOfDC; dcindex++) {
	        			...
	        			if (task.getMi() > 0 && miSeconds >= Math.max(ioSeconds, bwSeconds)) {
	        				io_mu = task.getIo()*mi_mu/task.getMi();
	        				bw_mu = TotalTransferDataSize[xindex]*mi_mu/task.getMi();
	        			} else if (task.getIo() > 0 && ioSeconds >= Math.max(miSeconds, bwSeconds)) {
	        				mi_mu = task.getMi()*io_mu/task.getIo();
	        				bw_mu = TotalTransferDataSize[xindex]*io_mu/task.getIo();
	        				
	        			} else if (TotalTransferDataSize[xindex] > 0 && bwSeconds >= Math.max(miSeconds, ioSeconds)) {
	        				mi_mu = task.getMi()*bw_mu/TotalTransferDataSize[xindex];
	        				io_mu = task.getIo()*bw_mu/TotalTransferDataSize[xindex];
				...
				for(int taskindex = 0; taskindex < numberOfTask; taskindex++) {
					...
					for(int j=0; j<vmSize; j++) {
	        			...
	        			if (task.getMi() > 0 && miSeconds >= Math.max(ioSeconds, bwSeconds)) {
	        				iops = task.getIo()*mips/task.getMi();
	        				bwps = TotalTransferDataSize[bestxindex]*mips/task.getMi();
	        			} else if (task.getIo() > 0 && ioSeconds >= Math.max(miSeconds, bwSeconds)) {
	        				mips = task.getMi()*iops/task.getIo();
	        				bwps = TotalTransferDataSize[bestxindex]*iops/task.getIo();
	        				
	        			} else if (TotalTransferDataSize[bestxindex] > 0 && bwSeconds >= Math.max(miSeconds, ioSeconds)) {
	        				mips = task.getMi()*bwps/TotalTransferDataSize[bestxindex];
	        				iops = task.getIo()*bwps/TotalTransferDataSize[bestxindex];
